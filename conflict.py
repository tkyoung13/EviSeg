...
from datasets import cityscapes, kitti, tartanair_semantic, tartanair_trav, camvid
...
def setup_loader():
    ...
    if args.dataset == 'cityscapes':
        ...
        test_set = args.dataset_cls.TartanAir_Trav(args.mode, args.split,
                                         ...
                                         target_transform=target_transform,
                                         cv_split=args.cv_split)
    elif args.dataset == 'camvid':  # Add support for CamVid dataset
        args.dataset_cls = camvid
        eval_mode_pooling = False
        eval_scales = None
        test_set = args.dataset_cls.CAMVID(args.mode, args.split,
                                           transform=val_input_transform,
                                           target_transform=target_transform,
                                           cv_split=args.cv_split)
...
class RunEval():
    ...
    def inf(self, imgs, img_names, gt, inference, net, scales, pbar, base_img, net_cfl):
        ...
        # Retrieve the weights of the penultimate layer of the model. 获取模型倒数第二层的权重
       
        last_layer = net.module.final[-1]

        # Retrieve the weights and biases of the penultimate layer.
        last_layer_weight = torch.squeeze(last_layer.weight)
        last_layer_weight = last_layer_weight.t()
        last_layer_bias = torch.zeros(11)
        _beta_0_fc2 = nnbelief.beta_0(last_layer_bias).to('cuda')# [11]
        _beta_fc2=nnbelief.beta_jk(last_layer_weight)# [256,11]
        
        input_data = imgs.to('cuda')
        # intermediate_model = intermediate_model.to('cuda')
        # Next, pass the input data and obtain the output. 然后，传递输入数据并获取输出
        
        # intermediate_output1 = net_cfl(imgs)
        x = torch.squeeze(net_cfl(input_data))
        x = x.reshape(256, 360*480)
        # x = x.reshape(256, 720*960)
        x = x.t()
        # import pdb; pdb.set_trace()
        _a_fc2 = nnbelief.alpha_jk(_beta_fc2,_beta_0_fc2,x)
        cfl = nnbelief.conflict(_beta_fc2,_a_fc2,x)
        # ig = nnbelief.ignorance(_beta_fc2,_a_fc2,x)
        k1 = cfl.reshape([360,480])
        # k1_cpu = k1.detach().cpu().numpy()
        k2 = cv2.resize(k1.detach().cpu().numpy(),(960,720), interpolation=cv2.INTER_NEAREST)
        # plt.axis('off')
        # plt.tight_layout()
        # plt.imshow(k2,cmap=plt.cm.gray)
        # plt.savefig('results/conflict/'+img_names[0], bbox_inches='tight')
        sns_plot = sns.heatmap(k2,cmap="viridis", annot=False, cbar=False,linecolor='grey',center=0.4)
        plt.axis("off")
        plt.tight_layout()
        sns_plot.get_figure().savefig('results/conflict2/'+img_names[0])
        # sns_plot.get_figure().savefig('test_image2')
        # import pdb; pdb.set_trace()
...
def main():
    ...
    # 自己写net_cfl
    # net_cfl = network.get_net(args, criterion=None)
    net_cfl = get_model(network="network.deepv3.DeepWV3Plus_cfl", num_classes=args.dataset_cls.num_classes,
                    criterion=None, criterion2=None, tasks=None)
    num_params = sum([param.nelement() for param in net.parameters()])
    logging.info('Model params = {:2.1f}M'.format(num_params / 1000000))

    net_cfl = net_cfl.cuda()
    net_cfl = torch.nn.DataParallel(net_cfl).cuda()
    net_cfl, _ = restore_snapshot(net_cfl, optimizer=None,
                            snapshot=args.snapshot, restore_optimizer_bool=False)
    net_cfl.eval()
    ...
    for iteration, data in enumerate(pbar):
        ...
        runner.inf(imgs, img_names, gt, inference, net, scales, pbar, base_img, net_cfl)
